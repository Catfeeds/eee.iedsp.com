...
// $devOnline = pdo_fetch("SELECT count(a.id) as COUNT FROM ".tablename('fz_dev_info')." as a,".tablename('users')." b WHERE a.bkTime is not null and a.bkTime>(".time()."-a.heartbeat) and a.username=b.username OR b.levelstr like '".$devuser['levelstr']."%' ".$where, array());
$devOnline = pdo_fetch("SELECT count(a.id) as COUNT FROM ".tablename('fz_dev_info')." as a,".tablename('users')." b WHERE a.bkTime is not null and a.bkTime>(".time()."-a.heartbeat) and a.username=b.username".$where, array());
$online=$devOnline['COUNT']; 

// $devUnOnline = pdo_fetch("SELECT count(a.id) as COUNT FROM ".tablename('fz_dev_info')." as a,".tablename('users')." b WHERE (a.bkTime is null or a.bkTime<=(".time()."-a.heartbeat)) and a.username=b.username OR b.levelstr like '".$devuser['levelstr']."%' ".$where, array());
$devUnOnline = pdo_fetch("SELECT count(a.id) as COUNT FROM ".tablename('fz_dev_info')." as a,".tablename('users')." b WHERE (a.bkTime is null or a.bkTime<=(".time()."-a.heartbeat)) and a.username=b.username".$where, array());
...
// $sql_day_order="select count(id) as COUNT,sum(chanceMoney) as totalMoney from ims_fz_ye_detail where chanceType=1 and chanceTime>='$beginToday' and chanceTime<='$endToday' OR username='".$devuser['username']."'".$where;/
$sql_day_order="select count(id) as COUNT,sum(chanceMoney) as totalMoney from ims_fz_ye_detail as a where chanceType=1 and chanceTime>='$beginToday' and chanceTime<='$endToday' ".$where;
