...
if(!empty($_GPC['loadPage'])){
	...
	{ 
		...
		if(!empty($mode['buyuser'])){
			$wxUser= pdo_fetch("select avatar from ims_mc_members where uid=".$mode['buyuser']); 
			$logo=$wxUser['avatar'];
		}
		...
		if($paysend=='4'){
			 $paysend_str='<span style="color:red;">(已申请退款)<span>';
		}
		if($paysend=='5'){
			 $paysend_str='<span style="color:red;">(退款成功)<span>';
		}
